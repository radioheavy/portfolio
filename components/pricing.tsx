...
import { Check, Zap, Crown, Rocket, FileText, Download, Printer, Mail, Calculator, Sliders, TrendingUp, ArrowRight } from "lucide-react"
        ...
        <div className="bg-black rounded-2xl overflow-hidden border border-border/50 shadow-2xl flex-1 flex flex-col min-h-0">
          ...
          <div className="bg-card border-b border-border/50 px-6 py-4 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold mb-1">Invoice Generator</h1>
                <p className="text-xs text-muted-foreground">Create professional quotes and invoices</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button className="px-3 py-1.5 bg-secondary border border-border/30 rounded-lg hover:border-primary/50 transition-colors flex items-center gap-2">
                <Calculator className="w-4 h-4 text-primary" />
                <span className="text-xs text-foreground">Calculate</span>
              </button>
              {showInvoice && (
                <>
                  <button className="px-3 py-1.5 bg-secondary border border-border/30 rounded-lg hover:border-primary/50 transition-colors flex items-center gap-2">
                  ...
                  <button className="px-3 py-1.5 bg-secondary border border-border/30 rounded-lg hover:border-primary/50 transition-colors flex items-center gap-2">
                    ...
                    <span className="text-xs text-foreground">Print</span>
                  </button>
                </>
              )}
            </div>
          ...
          <div className="flex-1 flex overflow-hidden min-h-0">
            ...
            <div className="w-80 border-r border-border/50 bg-secondary flex flex-col flex-shrink-0">
              <div className="px-4 py-3 border-b border-border/30">
                <h2 className="text-sm font-semibold text-foreground mb-1">Select Plan</h2>
                <p className="text-xs text-muted-foreground">Choose your service package</p>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-2">
                        <div className={`p-2 rounded-lg ${plan.color.replace("bg-", "bg-").replace("bg-", "bg-")} ${plan.color}/20`}>
                          <Icon className={`w-5 h-5 ${plan.color.replace("bg-", "text-")}`} />
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <h3 className="text-sm font-bold text-foreground">{plan.name}</h3>
                            {plan.popular && (
                              <span className="px-2 py-0.5 bg-primary rounded-full text-[10px] font-bold text-background">
                                POPULAR
                              </span>
                            )}
                          </div>
              ...
              {/* Calculator Controls */}
              <div className="border-t border-border/30 p-4 bg-secondary/50 space-y-4">
                <div>
                  <label className="text-xs font-semibold text-foreground mb-2 block">Quantity</label>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setQuantity(Math.max(1, quantity - 1))}
                      className="w-8 h-8 rounded-lg bg-secondary border border-border/30 hover:border-primary/50 flex items-center justify-center text-foreground"
                    >
                      -
                    </button>
                    <input
                      ...
                      className="flex-1 px-3 py-2 bg-secondary border border-border/30 rounded-lg text-sm text-foreground text-center"
                      min="1"
                    />
                    <button
                ...
                <div>
                  <label className="text-xs font-semibold text-foreground mb-2 block">Discount (%)</label>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="flex-1"
                    />
                    <span className="text-xs font-bold text-primary w-12 text-right">{discount}%</span>
                  </div>
                </div>

                <button
                  onClick={() => setShowInvoice(true)}
                  className="w-full py-3 bg-primary text-background font-semibold rounded-lg hover:bg-primary/90 transition-colors flex items-center justify-center gap-2"
                >
            ...
            <div className="flex-1 overflow-auto min-h-0 bg-secondary/30">
              {showInvoice ? (
                <div className="p-8 lg:p-12 max-w-4xl mx-auto">
                  {/* Invoice Document */}
                  <div className="bg-white border-2 border-gray-300 rounded-lg shadow-2xl p-8 lg:p-12">
                    {/* Invoice Header */}
                    <div className="flex items-start justify-between mb-8 pb-6 border-b-2 border-gray-300">
                      <div>
                        ...
                        <div className="text-sm text-gray-600">Invoice #INV-2024-001</div>
                        <div className="text-sm text-gray-600">Date: {new Date().toLocaleDateString("tr-TR")}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-lg font-bold text-gray-900 mb-1">Your Company</div>
                        <div className="text-xs text-gray-600">123 Business St.</div>
                        <div className="text-xs text-gray-600">Istanbul, Turkey</div>
                      </div>
                    </div>
                    ...
                    <div className="mb-8">
                      ...
                      <div className="text-sm text-gray-700">Client Company Name</div>
                      ...
                      <table className="w-full">
                        ...
                        <tbody>
                          <tr className="border-b border-gray-200">
                            <td className="py-4">
                              <div className="font-semibold text-gray-900">{plans[selectedPlan].name} Plan</div>
                ...
                <div className="h-full flex items-center justify-center p-8">
                  <div className="text-center space-y-4 max-w-md">
                    ...
                    <div>
                      <h3 className="text-xl font-semibold text-foreground mb-2">Generate Your Invoice</h3>
